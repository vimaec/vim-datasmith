...
public:
	...
	const C& operator[](size_t inIndex) const { TestAssertDebugOnly(inIndex < mCount); return mData[inIndex];  }
	
	// Access to element  by it's index
	C& operator[](size_t inIndex) { TestAssertDebugOnly(mData != nullptr && inIndex < mCount); return mData[inIndex];  }
...
template<class C>
class TAttributeVector : public TVector<C>
{
public:
