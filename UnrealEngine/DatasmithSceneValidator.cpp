...
#ifndef UE_AC_TraceF
#define UE_AC_TraceF(...) printf(__VA_ARGS__)
#endif
...
FString FSceneValidator::GetElementsDescription(const IDatasmithElement& InElement) {
    ...
    return FString::Printf(TEXT("Types(%s), Name=\"%s\", Label=\"%s\""), *Types, InElement.GetName(), InElement.GetLabel());
}
...
void FSceneValidator::AddElements(const IDatasmithElement& InElement, FMapNameToUsage* IOMap) {
    ...
    if (NameToElementMap.Contains(ElementName)) {
        AddMessage(kError, TEXT("Elements with same\n\tNew Element %s\n\tOld Element %s"), *GetElementsDescription(InElement),
                   *GetElementsDescription(*NameToElementMap[ElementName]));
    } else {
...
const char* FSceneValidator::LevelName(TInfoLevel Level) {
    return Level == kBug ? "Bug" : Level == kError ? "Error" : Level == kWarning ? "Warning" : Level == kVerbose ? "Verbose" : "???????";
}

void FSceneValidator::PrintReports(TInfoLevel InLevel, FctReport InFctReport) {
...
void FSceneValidator::CheckElementsName() {
    ...
    for (Index = 0; Index < Count; ++Index) {
        const TSharedPtr<IDatasmithLevelSequenceElement>& LevelSequence = Scene->GetLevelSequence(Index); // No const getter
    ...
    for (Index = 0; Index < Count; ++Index) {
        const TSharedPtr<IDatasmithLevelVariantSetsElement>& LevelVariant = Scene->GetLevelVariantSets(Index); // No const getter
        ...
        if (MetaData.IsValid()) {
            if (MetaData->GetAssociatedElement().IsValid()) {
                ...
                if (associated.IsA(EDatasmithElementType::Actor)) {
                    ...
                    AddMessage(kError, TEXT("Metadata %d %s associated to an unexpected element %s"), Index, *GetElementsDescription(*MetaData),
                               *GetElementsDescription(associated));
                }
            } else {
...
void FSceneValidator::CheckActorsDependances(const IDatasmithActorElement& InActor) {
...

    {
        ...
        for (Index = 0; Index < Count; ++Index) {
            ...
            if (bTagIsAlreadyInSet) {
                AddMessage(kError, TEXT("Tag \"%s\" present twice for actor %s"), Tag, *GetElementsDescription(InActor));
            }
        }
    }
    ...
    if (InActor.IsA(EDatasmithElementType::StaticMeshActor)) {
        ...
        if (*MeshName) {
            FUsage* MeshUsage = MeshesUsages.Find(MeshName);
            if (MeshUsage == nullptr) {
                MeshesUsages.Add(MeshName, {false, true});
                AddMessage(kError, TEXT("Unknown mesh \"%s\" for actor %s"), MeshName, *GetElementsDescription(InActor));
            } else {
        ...
        for (Index = 0; Index < Count; ++Index) {
            ...
            if (MaterialOverride.IsValid()) {
                ...
                if (bOverloadIsAlreadyInSet) {
                    AddMessage(kError, TEXT("Multiple overload for same id (%d) for actor %d %s"), MaterialId, Index, *GetElementsDescription(MeshActor));
                } else {
                    if (MeshPtr != nullptr) {
                ...
                if (!MaterialUsage.bIsRefered) {
                    ...
                    if (!MaterialUsage.bExist) {
                        AddMessage(kError, TEXT("Unknown material \"%s\" for material overloaded for actor %d %s"), MaterialOverride->GetName(), Index,
